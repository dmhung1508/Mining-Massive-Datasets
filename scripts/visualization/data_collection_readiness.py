...
from social_lsh.datasets import default_twitter_dataset_path
...
def build_week1_dashboard(
    ...
    fig.text(
        0.5,
        0.94,
        "Telegram export in jupyter/output + Twitter dataset/parquet status for collection and cleaning phase",
        ha="center",
        color=MUTED,
        fontsize=11,
    )
...
def main() -> None:
    ...
    parser.add_argument(
        "--output-png",
        default=str(repo_root / "jupyter" / "output" / "visuals" / "week1_overview.png"),
    )
    parser.add_argument(
        "--summary-json",
        default=str(repo_root / "jupyter" / "output" / "visuals" / "week1_summary.json"),
    )
